...
$(document).ready(function() {//ready:在DOM加载完成时运行的代码
	...
	$("#vanclPassword").keypress(function(event) {//keypress键盘按键
		if (event.keyCode == 13) {//keyCode == 13是键盘回车
	...
	$("#gotoReg").click(function() {
		...
		window.location = "logon.html" + getUrlParam();
		return false;
	});
	...
	$("#vanclUserName").keypress(function(event) {
		if (event.keyCode == 13) {
			...
			if (userName.val() != "Email/手机号") {
				...
				$("#vanclPassword").focus();//当页面加载后将 id 为 'vanclPassword'的元素设置焦点:
			}
		}
	});
...
function validateVJiaInfo() {
	...
	if (userName.val().trim() == "" || userName.val().trim() == "Email/手机号") {//val:获取文本框PartnerUserName中的值,trim去掉字符串起始和结尾的空格
		userName.focus();//将 id 为 'PartnerUserName' 的元素设置焦点
		var userNameError = $("#vjiaUserNameError");
		userNameError.text(message.UninputUserName);//设置vjiaUserNameError元素的文本内容为message.UninputUserName
		userNameError.css("visibility", "visible");
		isValid = false;
	} else if (password.val().trim() == "") {
		password.focus();
...
function login() {
	...
	var loginUrl = document.location.protocol + "//" + domain + "/login/XmlCheckUserName.ashx";//protocol：协议  location:位置
	//http://login.vancl.com/login/XmlCheckUserName.ashx
...
function loginVJiaUser() {
	...
	loginButton.attr("class", "loading");//为id名为vjiaLogin的函数绑定loading
	loginButton.html("<img src='https://ssl.vanclimg.com/login/loading.gif'/>正在验证...");//设置vjiaLogin的内容
	loginButton.unbind("click");//将段落的click事件取消绑定
